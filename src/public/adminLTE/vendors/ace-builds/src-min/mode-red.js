define(
  ...
  function (e, t, n) {
        ...
        this.$rules = {
          start: [
            ...
            { token: 'keyword', regex: '/local|/external' },
            {
              ...
              regex:
                '#(if|either|switch|case|include|do|macrolocal|reset|process|trace)',
            },
            {
            ...
            { token: 'variable.language', regex: 'this' },
            {
              ...
              regex:
                '(?:while|if|return|case|unless|either|until|loop|repeat|forever|foreach|forall|switch|break|continue|exit)(?![-!?\\w~])',
            },
            {
            ...
            { token: 'variable.refinement', regex: /\/\w[-\w'*.?!]*/ },
            {
              ...
              regex:
                '\\b(?:window|base|button|text|field|area|check|radio|progress|slider|camera|text-list|drop-list|drop-down|panel|group-box|tab-panel|h1|h2|h3|h4|h5|box|image|init)(?![-!?\\w~])',
            },
            {
              token: 'keyword.view.event',
              regex:
                '\\b(?:detect|on-detect|time|on-time|drawing|on-drawing|scroll|on-scroll|down|on-down|up|on-up|mid-down|on-mid-down|mid-up|on-mid-up|alt-down|on-alt-down|alt-up|on-alt-up|aux-down|on-aux-down|aux-up|on-aux-up|wheel|on-wheel|drag-start|on-drag-start|drag|on-drag|drop|on-drop|click|on-click|dbl-click|on-dbl-click|over|on-over|key|on-key|key-down|on-key-down|key-up|on-key-up|ime|on-ime|focus|on-focus|unfocus|on-unfocus|select|on-select|change|on-change|enter|on-enter|menu|on-menu|close|on-close|move|on-move|resize|on-resize|moving|on-moving|resizing|on-resizing|zoom|on-zoom|pan|on-pan|rotate|on-rotate|two-tap|on-two-tap|press-tap|on-press-tap|create|on-create|created|on-created)(?![-!?\\w~])',
            },
            {
              token: 'keyword.view.option',
              regex:
                '\\b(?:all-over|center|color|default|disabled|down|flags|focus|font|font-color|font-name|font-size|hidden|hint|left|loose|name|no-border|now|rate|react|select|size|space)(?![-!?\\w~])',
            },
            {
              token: 'constant.other.colour',
              regex:
                '\\b(?:Red|white|transparent|black|gray|aqua|beige|blue|brick|brown|coal|coffee|crimson|cyan|forest|gold|green|ivory|khaki|leaf|linen|magenta|maroon|mint|navy|oldrab|olive|orange|papaya|pewter|pink|purple|reblue|rebolor|sienna|silver|sky|snow|tanned|teal|violet|water|wheat|yello|yellow|glass)(?![-!?\\w~])',
            },
    ...
    r.inherits(s, i), (t.RedHighlightRules = s);
  },
),
  define(
  ...
  define(
    ...
    function (e, t, n) {
      ...
      var r = e('../lib/oop'),
        i = e('./text').Mode,
        s = e('./red_highlight_rules').RedHighlightRules,
