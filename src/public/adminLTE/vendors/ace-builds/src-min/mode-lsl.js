define(
  ...
  function (e, t, n) {
    ...
    function s() {
      var e = this.createKeywordMapper(
        {
          'constant.language.float.lsl':
            'DEG_TO_RAD|PI|PI_BY_TWO|RAD_TO_DEG|SQRT2|TWO_PI',
      ...
      (this.$rules = {
        start: [
          { token: 'comment.line.double-slash.lsl', regex: '\\/\\/.*$' },
          {
          ...
          },
          {
            ...
            next: [
              { token: 'constant.character.escape.lsl', regex: /\\[tn"\\]/ },
            ],
          },
          {
            token: 'constant.numeric.lsl',
            regex:
              '(0[xX][0-9a-fA-F]+|[+-]?[0-9]+(?:(?:\\.[0-9]*)?(?:[eE][+-]?[0-9]+)?)?)\\b',
          },
          {
            token: 'entity.name.state.lsl',
            regex: '\\b((state)\\s+[A-Za-z_]\\w*|default)\\b',
          },
          { token: e, regex: '\\b[a-zA-Z_][a-zA-Z0-9_]*\\b' },
          {
          ...
          { token: 'punctuation.operator.lsl', regex: '\\,|\\;' },
          { token: 'paren.lparen.lsl', regex: '[\\[\\(\\{]' },
          { token: 'paren.rparen.lsl', regex: '[\\]\\)\\}]' },
          { token: 'text.lsl', regex: '\\s+' },
        ],
        comment: [
          { token: 'comment.block.end.lsl', regex: '\\*\\/', next: 'start' },
  ...
  define(
    ...
    function (e, t, n) {
      'use strict';
      var r = e('./lsl_highlight_rules').LSLHighlightRules,
          ...
          (this.lineCommentStart = ['//']),
            ...
            (this.getNextLineIndent = function (e, t, n) {
              var r = this.$getIndent(t),
                ...
                s = i.tokens,
                o = i.state;
              if (s.length && s[s.length - 1].type === 'comment.block.lsl')
                return r;
              if (e === 'start') {
                var u = t.match(/^.*[\{\(\[]\s*$/);
                u && (r += n);
