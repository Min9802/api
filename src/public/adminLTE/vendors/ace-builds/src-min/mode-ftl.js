  ...
  define(
    ...
    function (e, t, n) {
      ...
      var r = e('../lib/oop'),
        ...
        o = function () {
          var e =
              '\\?|substring|cap_first|uncap_first|capitalize|chop_linebreak|date|time|datetime|ends_with|html|groups|index_of|j_string|js_string|json_string|last_index_of|length|lower_case|left_pad|right_pad|contains|matches|number|replace|rtf|url|split|starts_with|string|trim|upper_case|word_list|xhtml|xml',
            t = 'c|round|floor|ceiling',
            n = 'iso_[a-z_]+',
            r =
              'first|last|seq_contains|seq_index_of|seq_last_index_of|reverse|size|sort|sort_by|chunk',
            i = 'keys|values',
            s =
              'children|parent|root|ancestors|node_name|node_type|node_namespace',
            o =
              'byte|double|float|int|long|short|number_to_date|number_to_time|number_to_datetime|eval|has_content|interpret|is_[a-z_]+|namespacenew',
            u = e + t + n + r + i + s + o,
            a = 'default|exists|if_exists|web_safe',
            f =
              'data_model|error|globals|lang|locale|locals|main|namespace|node|current_node|now|output_encoding|template_name|url_escaping_charset|vars|version',
            l = 'gt|gte|lt|lte|as|in|using',
            c = 'true|false',
            h =
              'encoding|parse|locale|number_format|date_format|time_format|datetime_format|time_zone|url_escaping_charset|classic_compatible|strip_whitespace|strip_text|strict_syntax|ns_prefixes|attributes';
          this.$rules = {
            start: [
              { token: 'constant.character.entity', regex: /&[^;]+;/ },
              { token: 'support.function', regex: '\\?(' + u + ')' },
              { token: 'support.function.deprecated', regex: '\\?(' + a + ')' },
              { token: 'language.variable', regex: '\\.(?:' + f + ')' },
              { token: 'constant.language', regex: '\\b(' + c + ')\\b' },
              { token: 'keyword.operator', regex: '\\b(?:' + l + ')\\b' },
              { token: 'entity.other.attribute-name', regex: h },
              { token: 'string', regex: /['"]/, next: 'qstring' },
              {
                token: function (e) {
                  return e.match(
      ...
      var u = function () {
        ...
        var e =
          ...
          t = [
            { token: 'comment', regex: '<#--', next: 'ftl-dcomment' },
            { token: 'string.interpolated', regex: '\\${', push: 'ftl-start' },
            {
          ...
          n = [
            ...
            { token: 'string.interpolated', regex: '}', next: 'pop' },
          ];
        for (var r in this.$rules)
          this.$rules[r].unshift.apply(this.$rules[r], t);
        this.embedRules(o, 'ftl-', n, ['start']),
          this.addRules({
            'ftl-dcomment': [
              { token: 'comment', regex: '-->', next: 'pop' },
              { defaultToken: 'comment' },
      ...
      r.inherits(u, i), (t.FtlHighlightRules = u);
    },
  ),
  define(
    'ace/mode/ftl',
    ...
    function (e, t, n) {
      ...
      var r = e('../lib/oop'),
        i = e('./text').Mode,
        s = e('./ftl_highlight_rules').FtlHighlightRules,
        o = function () {
