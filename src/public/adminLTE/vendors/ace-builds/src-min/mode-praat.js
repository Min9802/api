define(
  'ace/mode/praat_highlight_rules',
  [
    'require',
    'exports',
    'module',
  ...
  function (e, t, n) {
    ...
    var r = e('../lib/oop'),
      ...
      s = function () {
        var e =
            'if|then|else|elsif|elif|endif|fi|endfor|endproc|while|endwhile|repeat|until|select|plus|minus|assert|asserterror',
          t =
            'macintosh|windows|unix|praatVersion|praatVersion\\$pi|undefined|newline\\$|tab\\$|shellDirectory\\$|homeDirectory\\$|preferencesDirectory\\$|temporaryDirectory\\$|defaultDirectory\\$',
        ...
        this.$rules = {
          start: [
            ...
            { token: 'entity.name.type', regex: '(' + i + ')' },
            { token: 'variable.language', regex: '(' + t + ')' },
            {
              token: ['support.function', 'text'],
              regex: '((?:' + r + ')\\$?)(\\s*(?::|\\())',
            },
            { token: 'keyword', regex: /(\bfor\b)/, next: 'for' },
            { token: 'keyword', regex: '(\\b(?:' + e + ')\\b)' },
            { token: 'string', regex: /"[^"]*"/ },
            { token: 'string', regex: /"[^"]*$/, next: 'brokenstring' },
            {
          ...
          form: [
            ...
            { token: ['keyword', 'string'], regex: /(comment\s+)(.*)/ },
            { token: 'keyword', regex: 'endform', next: 'start' },
          ],
          for: [
            {
            ...
            { token: ['keyword', 'text'], regex: /(from|to)(\s+\S+\s*)/ },
            { token: 'text', regex: /$/, next: 'start' },
          ],
          brokenstring: [
            { token: ['text', 'string'], regex: /(\s*\.{3})([^"]*)/ },
