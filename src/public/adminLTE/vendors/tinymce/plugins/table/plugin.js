...
(function () {
  ...
  var all = function (selector, scope) {
    ...
    return bypassSelector(base)
      ? []
      : map(base.querySelectorAll(selector), SugarElement.fromDom);
  };
    ...
    each(list, function (rowData) {
      if (rowData.section === 'colgroup') {
        ...
        each(rowData.cells, function (rowCell) {
          ...
          for (
            var occupiedColumnPosition = 0;
            occupiedColumnPosition < rowCell.colspan;
            occupiedColumnPosition++
          ) {
            for (
              var occupiedRowPosition = 0;
              occupiedRowPosition < rowCell.rowspan;
              occupiedRowPosition++
            ) {
  ...
  var getBounds = function (detailA, detailB) {
    return bounds(
      ...
      Math.max(
        detailA.row + detailA.rowspan - 1,
        detailB.row + detailB.rowspan - 1,
      ),
      Math.max(
        detailA.column + detailA.colspan - 1,
        detailB.column + detailB.colspan - 1,
      ),
    );
  };
  ...
  var moveBy = function (warehouse, cell, row, column) {
    return Warehouse.findItem(warehouse, cell, eq$1).bind(function (detail) {
      ...
      var startCol =
        column > 0 ? detail.column + detail.colspan - 1 : detail.column;
  ...
  var nestedIntercepts = function (table, first, firstTable, last, lastTable) {
    var warehouse = getWarehouse(table);
    var optStartCell = eq$1(table, firstTable)
      ? Optional.some(first)
      : parentCell(warehouse, first);
    var optLastCell = eq$1(table, lastTable)
      ? Optional.some(last)
      : parentCell(warehouse, last);
  ...
  function DomUniverse() {
    ...
    var isNonEditable = function (element) {
      return (
        isElement(element) && get$2(element, 'contenteditable') === 'false'
      );
    };
  ...
  var oneAll = function (universe, look, elements) {
    return elements.length > 0
      ? all$1(universe, look, elements, unsafeOne)
      : Optional.none();
  };
  ...
  var identify = function (start, finish, isRoot) {
    var getIsRoot = function (rootTable) {
      return function (element) {
        return (
          (isRoot !== undefined && isRoot(element)) || eq$1(element, rootTable)
        );
      };
    };
          ...
          if (eq$1(startTable, finishTable)) {
            ...
            return ancestors$4(start, finish).shared.bind(function (lca) {
              return closest$1(lca, 'table', isRoot).bind(function (lcaTable) {
                var finishAncestorCells = ancestors$1(
                  finish,
                  'td,th',
                  getIsRoot(lcaTable),
                );
                var finishCell =
                  finishAncestorCells.length > 0
                    ...
                    : finish;
                var startAncestorCells = ancestors$1(
                  start,
                  'td,th',
                  getIsRoot(lcaTable),
                );
  ...
  var getEdges = function (
    ...
    return descendant$1(container, firstSelectedSelector).bind(function (
      first,
    ) {
      return descendant$1(container, lastSelectedSelector).bind(function (
        last,
      ) {
    ...
    return getEdges(
      ...
      return firstAncestor.bind(function (fA) {
        return lastAncestor.bind(function (lA) {
          return eq$1(fA, lA)
            ? getBox$1(edges.table, edges.first, edges.last)
            : Optional.none();
        });
      });
    });
  ...
  var Selections = function (lazyRoot, getStart, selectedSelector) {
    var get = function () {
      return retrieve$1(lazyRoot(), selectedSelector).fold(
        function () {
    ...
    for (var i = 0; i < totalRows; i++) {
      ...
      for (var j = 0; j < totalColumns; j++) {
        if (
          ...
          var needCell = Warehouse.getAt(house, i, j)
            .filter(isSelected)
            .isNone();
          if (needCell) {
  ...
  var isCursorPosition = function (elem) {
    ...
    return (
      hasCursorPosition || contains(elementsWithCursorPosition, name(elem))
    );
  };
  ...
  var cloneAppropriateAttributes = function (original, clone) {
    each$1(transferableAttributes, function (validAttributes, attributeName) {
      return getOpt(original, attributeName)
  ...
  var unmergable = function (selections) {
    ...
    return candidates.length > 0 && forall(candidates, hasRowOrColSpan)
    ...
    return cata(
      ...
      function (cells) {
        if (cells.length <= 1) {
          ...
          return retrieveBox(
            table,
            ephemera.firstSelectedSelector,
  ...
  var determine = function (input, column, step, tableSize, resize) {
    ...
    var onLeft = function (index, next) {
      return resize.calcLeftEdgeDeltas(
        result,
        index,
        next,
        step,
        tableSize.minCellWidth(),
        tableSize.isRelative,
      );
    };
    var onMiddle = function (prev, index, next) {
      return resize.calcMiddleDeltas(
        result,
        prev,
        index,
        next,
    ...
    var onRight = function (prev, index) {
      return resize.calcRightEdgeDeltas(
        result,
        prev,
        index,
        step,
  ...
  var getHeight = function (cell) {
    return needManualCalc()
      ...
      : getProp(cell, 'height', get$6(cell));
  };
  var getWidth = function (cell) {
    return needManualCalc()
      ? getCalculatedWidth(cell)
      : getProp(cell, 'width', get$5(cell));
  };
  ...
  var normalizePixelSize = function (value, cell, getter, setter) {
    ...
    return endsWith(value, '%') && name(cell) !== 'table'
  ...
  var getRaw$1 = function (cell, property, getter) {
    return getRaw(cell, property).fold(
      function () {
  ...
  var getDimension = function (
    ...
    return cellOpt.filter(filter).fold(
      function () {
  ...
  var getWidthFrom = function (
    ...
    var columns = Warehouse.hasColumns(warehouse)
      ? justCols(warehouse)
      : columnCells;
    var backups = [Optional.some(width.edge(table))].concat(
      map(width.positions(columnCells, table), function (pos) {
  ...
  var getHeightFrom = function (
    ...
    var backups = [Optional.some(direction.edge(table))].concat(
      map(direction.positions(rows, table), function (pos) {
        return pos.map(function (p) {
          return p.y;
        });
      }),
  ...
  var adjustWidth = function (table, delta, index, resizing, tableSize) {
    ...
    var clampedStep = resizing.clampTableDelta(
      widths,
      index,
      step,
      tableSize.minCellWidth(),
  ...
  var surround = function (sizes, startIndex, endIndex, results, f) {
    return f(sizes.slice(0, startIndex))
      .concat(results)
      .concat(f(sizes.slice(endIndex)));
  };
  ...
  var resizeTable = function () {
    ...
    var calcLeftEdgeDeltas = function (
      sizes,
      index,
      next,
      delta,
    ...
    var calcRightEdgeDeltas = function (
      sizes,
      _prev,
      index,
      delta,
  ...
  var only = function (element, isResizable) {
    var parent = Optional.from(element.dom.documentElement)
  ...
  var validateFor = function (suffix, type, value) {
    ...
    return rawAmount === amount.toString()
      ? type(amount)
      : adt$1.invalid(value);
  };
  ...
  var fromRawEvent = function (rawEvent) {
    var target = SugarElement.fromDom(
  ...
  var resizableColumns = function (warehouse, isResizable) {
    ...
    range(warehouse.grid.columns, function (index) {
      var colElmOpt = Warehouse.getColumnAt(warehouse, index).map(function (
        col,
      ) {
        return col.element;
      });
  ...
  var refreshGrid = function (warhouse, wire, table, rows, cols) {
    ...
    var resizableRowBars =
      rowPositions.length > 0 ? resizableRows(warhouse, isResizable) : [];
    ...
    var resizableColBars =
      colPositions.length > 0 ? resizableColumns(warhouse, isResizable) : [];
  ...
  var BarManager = function (wire) {
    ...
    var findClosestEditableTable = function (target) {
      return closest$1(target, 'table', isRoot).filter(function (table) {
        return findClosestContentEditable(table, isRoot).exists(
  ...
  var fireTableSelectionChange = function (
    editor,
    cells,
    start,
    finish,
  ...
  var fireTableModified = function (editor, table, data) {
    editor.fire(
      'TableModified',
      __assign(__assign({}, data), { table: table }),
    );
  };
  ...
  var isPercentagesForced = function (editor) {
    return (
      ...
      getTableResponseWidth(editor) === true
    );
  };
  var isPixelsForced = function (editor) {
    return (
      getTableSizingMode(editor) === 'fixed' ||
      getTableResponseWidth(editor) === false
    );
  };
  ...
  var getColumnResizingBehaviour = function (editor) {
    ...
    var givenMode = editor.getParam(
      'table_column_resizing',
      defaultColumnResizingBehaviour,
      'string',
    );
  ...
  var hasObjectResizing = function (editor) {
    ...
    return isString(objectResizing)
  ...
  var get$9 = function (editor, table) {
    if (isPercentagesForced(editor)) {
      var width = getRawWidth$1(editor, table.dom)
        .filter(isPercentage)
        .getOrThunk(function () {
  ...
  var get$a = function (editor, isResizable) {
    return editor.inline
      ? ResizeWire.body(getBody$1(editor), createContainer(), isResizable)
      : ResizeWire.only(SugarElement.fromDom(editor.getDoc()), isResizable);
  };
  ...
  var getResizeHandler = function (editor) {
    ...
    var lazyWire = function () {
      return wire.getOr(
        ResizeWire.only(SugarElement.fromDom(editor.getBody()), isResizable),
      );
    };
    ...
    var lazyResizingBehaviour = function () {
      return isPreserveTableColumnResizing(editor)
        ? preserveTable()
        : resizeTable();
    };
    ...
    var afterCornerResize = function (table, origin, width) {
      ...
      if (width !== startW && startRawW !== '') {
        ...
        var tableSize = lazySizing(table);
        var col =
          isPreserveTableColumnResizing(editor) || isRightEdgeResize
            ? getNumColumns(table) - 1
            : 0;
      ...
      if (hasObjectResizing(editor) && hasTableResizeBars(editor)) {
        ...
        sz.events.beforeResize.bind(function (event) {
          ...
          fireObjectResizeStart(
            editor,
            rawTable,
            getPixelWidth$1(rawTable),
            getPixelHeight(rawTable),
            barResizerPrefix + event.type,
          );
        });
    ...
    editor.on('ObjectResizeStart', function (e) {
      ...
      if (isTable(targetElm)) {
        ...
        each(editor.dom.select('.mce-clonedresizable'), function (clone) {
          editor.dom.addClass(
            clone,
            'mce-' + getColumnResizingBehaviour(editor) + '-columns',
          );
        });
        ...
        startW = e.width;
        startRawW = isResponsiveForced(editor)
          ? ''
          : getRawWidth$1(editor, targetElm).getOr('');
      }
    });
  ...
  var scan = function (universe, element, direction) {
    if (
      (universe.property().isText(element) &&
        universe.property().getText(element).trim().length === 0) ||
  ...
  var freefallRtl = function (universe, element) {
    var candidate = scan(universe, element, universe.query().prevSibling).getOr(
      element,
    );
    ...
    return children.length > 0
      ? freefallRtl(universe, children[children.length - 1])
      : point(candidate, toEnd(universe, candidate));
  };
  ...
  var insert = function (table, selector, element) {
    last(children$2(table, selector)).fold(
      function () {
  ...
  var toGrid = function (warehouse, generators, isNew) {
    var grid = [];
    if (Warehouse.hasColumns(warehouse)) {
      var groupElementNew = map(
        Warehouse.justColumns(warehouse),
        function (column) {
    ...
    for (var rowIndex = 0; rowIndex < warehouse.grid.rows; rowIndex++) {
      ...
      for (
        var columnIndex = 0;
        columnIndex < warehouse.grid.columns;
        columnIndex++
      ) {
        var element = Warehouse.getAt(warehouse, rowIndex, columnIndex)
          .map(function (item) {
  ...
  var measure = function (startAddress, gridA, gridB) {
    if (
      ...
      startAddress.column > cellLength(gridA[0])
    ) {
      return Result.error(
  ...
  var generateElements = function (cells, row, generators) {
    var getGenerator =
      row.section === 'colgroup' ? generators.col : generators.cell;
  ...
  var colFill = function (grid, amount, generators) {
    return map(grid, function (row) {
      var newChildren = generateElements(
        range(amount, identity),
        row,
        generators,
      );
  ...
  var insertCols = function (index, gridA, gridB, generator, comparator) {
    ...
    return map(fittedOldGrid, function (gridRow, i) {
      var newCells = gridRow.cells
        .slice(0, index)
        .concat(fittedNewGrid[i].cells)
        .concat(gridRow.cells.slice(index, gridRow.cells.length));
      return setCells(gridRow, newCells);
    });
  };
  var insertRows = function (index, gridA, gridB, generator, comparator) {
    ...
    var _a = extractGridDetails(fittedOldGrid),
      ...
      oldRows = _a.rows;
    return oldCols
      .concat(oldRows.slice(0, index))
      .concat(fittedNewGrid)
      .concat(oldRows.slice(index, oldRows.length));
  };
  ...
  var insertRowAt = function (grid, index, example, comparator, substitution) {
    ...
    var between = mapCells(rows[example], function (ex, c) {
      var withinSpan =
        ...
        index < rows.length &&
        comparator(
          getCellElement(rows[index - 1], c),
          getCellElement(rows[index], c),
        );
      var ret = withinSpan
        ? getCell(rows[index], c)
        : elementnew(substitution(ex.element, comparator), true);
      return ret;
    });
    return cols.concat(before).concat([between]).concat(after);
  };
  var getElementFor = function (
    row,
    column,
    section,
    withinSpan,
    ...
    if (section === 'colgroup' || !withinSpan) {
      return elementnew(
        substitution(getCellElement(row, example), comparator),
        true,
      );
    } else {
  ...
  var insertColumnAt = function (
    ...
    return map(grid, function (row) {
      var withinSpan =
        index > 0 &&
        index < cellLength(row) &&
        comparator(getCellElement(row, index - 1), getCellElement(row, index));
      var sub = getElementFor(
        row,
        index,
        row.section,
        withinSpan,
  ...
  var replaceIn = function (grid, targets, comparator, substitution) {
    ...
    return map(grid, function (row) {
      return mapCells(row, function (cell) {
        return isTarget(cell)
          ? elementnew(substitution(cell.element, comparator), true)
          : cell;
      });
    });
  };
  ...
  var replaceColumn = function (grid, index, comparator, substitution) {
    var targets = bind(grid, function (row, i) {
      var alreadyAdded =
        notStartRow(grid, i, index, comparator) ||
        notStartColumn(row, index, comparator);
  ...
  var replaceRow = function (grid, index, comparator, substitution) {
    ...
    var targets = bind(targetRow.cells, function (item, i) {
      var alreadyAdded =
        notStartRow(rows, index, i, comparator) ||
        notStartColumn(targetRow, i, comparator);
  ...
  var transform$1 = function (scope, tag) {
    return function (generators) {
      ...
      var replaceOrInit = function (element, comparator) {
        return find$1(element, comparator).fold(
          function () {
  ...
  var merge$2 = function (cells) {
    ...
    var markCell = function (cell) {
      return last$1(cell)
        .bind(function (rightEdge) {
          var rightSiblingIsBlock = siblingIsBlock(rightEdge);
          return parent(rightEdge).map(function (parent) {
            return rightSiblingIsBlock === true ||
              isListItem(parent) ||
              isBr(rightEdge) ||
              (isBlock$1(parent) && !eq$1(cell, parent))
              ? []
  ...
  var opInsertColumnsBefore = function (
    grid,
    details,
    comparator,
    genWrappers,
  ...
  var opInsertColumnsAfter = function (grid, details, comparator, genWrappers) {
    ...
    var targetIndex =
      details[details.length - 1].column + details[details.length - 1].colspan;
    var columns = uniqueColumns(details);
    var newGrid = foldl(
      columns,
      function (newG, _row) {
  ...
  var opMakeRowHeader = function (grid, detail, comparator, genWrappers) {
    var newGrid = replaceRow(
      grid,
      detail.row,
      comparator,
      genWrappers.replaceOrInit,
    );
    return bundle(newGrid, detail.row, detail.column);
  };
  var opMakeRowsHeader = function (
    initialGrid,
    details,
    comparator,
    genWrappers,
  ...
  var opMakeColumnHeader = function (
    initialGrid,
    detail,
    comparator,
    genWrappers,
  ) {
    var newGrid = replaceColumn(
      initialGrid,
      detail.column,
      comparator,
      genWrappers.replaceOrInit,
  ...
  var opMakeColumnsHeader = function (
    initialGrid,
    details,
    comparator,
    genWrappers,
  ...
  var opUnmakeRowsHeader = function (
    initialGrid,
    details,
    comparator,
    genWrappers,
  ...
  var opUnmakeColumnHeader = function (
    initialGrid,
    detail,
    comparator,
    genWrappers,
  ...
  var opUnmakeColumnsHeader = function (
    initialGrid,
    details,
    comparator,
    genWrappers,
  ...
  var opPasteCells = function (grid, pasteDetails, comparator, _genWrappers) {
    ...
    var mergedGrid = merge$1(
      startAddress,
      grid,
      gridB,
      pasteDetails.generators,
      comparator,
    );
  ...
  var opPasteColsBefore = function (
    grid,
    pasteDetails,
    comparator,
    _genWrappers,
    ...
    var gridB = gridifyRows(
      pasteDetails.clipboard,
      pasteDetails.generators,
      context,
    );
    var mergedGrid = insertCols(
      index,
      grid,
      gridB,
      pasteDetails.generators,
    ...
    var cursor = elementFromGrid(
      mergedGrid,
      pasteDetails.cells[0].row,
      pasteDetails.cells[0].column,
    );
    return outcome(mergedGrid, cursor);
  };
  var opPasteColsAfter = function (
    grid,
    pasteDetails,
    comparator,
    _genWrappers,
    ...
    var rows = extractGridDetails(grid).rows;
    var index =
      pasteDetails.cells[pasteDetails.cells.length - 1].column +
      pasteDetails.cells[pasteDetails.cells.length - 1].colspan;
    var context = rows[pasteDetails.cells[0].row];
    var gridB = gridifyRows(
      pasteDetails.clipboard,
      pasteDetails.generators,
  ...
  var opPasteRowsBefore = function (
    grid,
    pasteDetails,
    comparator,
    _genWrappers,
  ...
  var opPasteRowsAfter = function (
    ...
    var rows = extractGridDetails(grid).rows;
    var index =
      pasteDetails.cells[pasteDetails.cells.length - 1].row +
      pasteDetails.cells[pasteDetails.cells.length - 1].rowspan;
    var context = rows[pasteDetails.cells[0].row];
    var gridB = gridifyRows(
      pasteDetails.clipboard,
      pasteDetails.generators,
  ...
  var opGetColumnType = function (table, target) {
    ...
    return details
      .bind(function (selectedCells) {
        ...
        var selectedColumnCells = flatten(
          map(house.all, function (row) {
  ...
  var insertColumnsBefore = run(
    opInsertColumnsBefore,
    onCells,
    resize,
    noop,
    Generators.modification,
  );
  var insertColumnsAfter = run(
    opInsertColumnsAfter,
    onCells,
    resize,
    noop,
    Generators.modification,
  );
  var eraseColumns = run(
    opEraseColumns,
    onCells,
    resize,
    prune,
  ...
  var makeColumnHeader = run(
    opMakeColumnHeader,
    onCell,
    noop,
    noop,
    Generators.transform('row', 'th'),
  );
  var makeColumnsHeader = run(
    opMakeColumnsHeader,
    onCells,
    noop,
    noop,
  ...
  var unmakeColumnHeader = run(
    opUnmakeColumnHeader,
    onCell,
    noop,
    noop,
    Generators.transform(null, 'td'),
  );
  var unmakeColumnsHeader = run(
    opUnmakeColumnsHeader,
    onCells,
    noop,
    noop,
    Generators.transform(null, 'td'),
  );
  var makeRowHeader = run(
    opMakeRowHeader,
    onCell,
    noop,
    noop,
    Generators.transform('col', 'th'),
  );
  var makeRowsHeader = run(
    opMakeRowsHeader,
    onCells,
    noop,
    noop,
  ...
  var getRowType = function (editor, elm) {
    return mapSectionNameToType(
      detectHeaderRow(editor, elm).fold(
        function () {
    ...
    if (newSectionName !== oldSectionName) {
      ...
      if (!sectionElm_1) {
        ...
        if (newSectionName === 'thead') {
          last(
            children$2(SugarElement.fromDom(tableElm), 'caption,colgroup'),
          ).fold(
            function () {
      ...
      if (
        newSectionName === 'tbody' &&
        oldSectionName === 'thead' &&
        sectionElm_1.firstChild
      ) {
  ...
  var switchSectionType = function (editor, rowElm, newType) {
    var determineHeaderRowType = function () {
      var allTableRows = table(SugarElement.fromDom(rowElm.cells[0]))
        .map(function (table) {
          return rows(table);
        })
        .getOr([]);
    ...
    if (newType === 'header') {
      ...
      var headerRowType =
        headerRowTypeSetting === 'auto'
          ? determineHeaderRowType()
          : headerRowTypeSetting;
      switchCellsType(
        editor,
        rowElm.cells,
        headerRowType === 'section' ? 'td' : 'th',
        'col',
      );
      switchRowSection(
        dom,
        rowElm,
        headerRowType === 'cells' ? 'tbody' : 'thead',
      );
    } else {
  ...
  var getSelectionStartFromSelector = function (selector) {
    return function (start) {
      ...
      var startCell =
        startCellName === 'col' || startCellName === 'colgroup'
          ? getSelectionStartCellFallback(start)
          : start;
      return closest$1(startCell, selector);
    };
  };
  var getSelectionStartCell = getSelectionStartFromSelector('th,td');
  var getSelectionStartCellOrCaption =
    getSelectionStartFromSelector('th,td,caption');
  var getCellsFromSelection = function (start, selections) {
    return getSelectionStartCell(start)
      .map(function (_cell) {
  ...
  var TableActions = function (editor, lazyWire, selections) {
    ...
    var execute = function (operation, guard, mutate, lazyWire) {
      return function (table, target) {
        ...
        return guard(table)
          ? operation(wire, table, target, generators, sizing).bind(function (
              result,
            ) {
    ...
    var insertColumnsBefore$1 = execute(
      insertColumnsBefore,
      always,
      halve,
      lazyWire,
    );
    var insertColumnsAfter$1 = execute(
      insertColumnsAfter,
      always,
      halve,
      lazyWire,
    ...
    var setTableCellType = function (editor, args) {
      return extractType(args, ['td', 'th']).each(function (type) {
        var cells = map(
          getCellsFromSelection(getSelectionStart(editor), selections),
          function (c) {
            return c.dom;
          },
        );
    ...
    var setTableRowType = function (editor, args) {
      return extractType(args, ['header', 'body', 'footer']).each(function (
        type,
      ) {
        map(
          getRowsFromSelection(getSelectionStart(editor), ephemera.selected),
          function (row) {
    ...
    var makeColumnsHeader$1 = execute(
      makeColumnsHeader,
      always,
      noop,
      lazyWire,
    );
    var unmakeColumnsHeader$1 = execute(
      unmakeColumnsHeader,
      always,
      noop,
      lazyWire,
  ...
  var createRow = function (columns, rowHeaders, columnHeaders, rowIndex) {
    ...
    for (var j = 0; j < columns; j++) {
      var td =
        rowIndex < rowHeaders || j < columnHeaders
          ? tableHeaderCell()
          : tableCell();
      if (j < columnHeaders) {
        set(td, 'scope', 'row');
  ...
  var render$1 = function (
    ...
    if (rowHeadersGoInThead && rowHeaders > 0) {
      ...
      append(table, thead);
      var theadRowHeaders =
        headerType === 'sectionCells' ? actualRowHeaders : 0;
      var theadRows = createRows(
        rowHeaders,
        columns,
        theadRowHeaders,
        columnHeaders,
  ...
  var insert$1 = function (editor, columns, rows, colHeaders, rowHeaders) {
    ...
    return descendant$1(getBody$1(editor), 'table[data-mce-id="__mce"]')
      .map(function (table) {
        if (isPixelsForced(editor)) {
          enforcePixels(editor, table);
        } else if (isResponsiveForced(editor)) {
          ...
          isPercentagesForced(editor) ||
          isPercentage$1(defaultStyles.width)
        ) {
          enforcePercentage(editor, table);
        }
        removeDataStyle(table);
  ...
  var insertTableWithDataValidation = function (
    editor,
    rows,
    columns,
    options,
  ...
  var setClipboardElements = function (setClipboard) {
    return function (elems) {
      var elmsOpt =
        elems.length > 0 ? Optional.some(fromDom$1(elems)) : Optional.none();
  ...
  var insertTable = function (editor) {
    return function (columns, rows, options) {
      ...
      var table = insertTableWithDataValidation(
        editor,
        rows,
        columns,
        options,
  ...
  var extractAdvancedStyles = function (dom, elm) {
    ...
    return {
      ...
      backgroundcolor: getRaw(element, 'background-color')
        .map(rgbToHex(dom))
        .getOr(''),
    };
  };
  ...
  var getAdvancedTab = function (dialogName) {
    ...
    var items =
      dialogName === 'cell' ? [borderWidth].concat(advTabItems) : advTabItems;
    return {
      title: 'Advanced',
      name: 'advanced',
      items: items,
  ...
  var extractDataFromSettings = function (editor, hasAdvTableTab) {
    ...
    var extractAdvancedStyleData = function (dom) {
      return {
        ...
        backgroundcolor: rgbToHex(dom)(
          get$1(style, 'background-color').getOr(''),
        ),
      };
    };
    var defaultData = {
    ...
    var getBorder = function () {
      ...
      return get$1(attrs, 'border').fold(
        function () {
          return {};
        },
        function (border) {
    ...
    var getCellPaddingCellSpacing = function () {
      var spacing = get$1(style, 'border-spacing')
        .or(get$1(attrs, 'cellspacing'))
        .fold(
          function () {
            return {};
          },
      ...
      var padding = get$1(style, 'border-padding')
        .or(get$1(attrs, 'cellpadding'))
        .fold(
          function () {
            return {};
          },
    ...
    var data = __assign(
      __assign(
        __assign(
          __assign(__assign(__assign({}, defaultData), style), attrs),
          advStyle,
        ),
        getBorder(),
      ),
    ...
    return __assign(
      {
        ...
        cellpadding:
          ...
          getTDTHOverallStyle(editor.dom, elm, 'padding'),
  ...
  var extractDataFromCellElement = function (
    editor,
    cell,
    hasAdvancedCellTab,
    column,
    ...
    return table(cells[0]).map(function (table) {
      ...
      return map(filtered, function (cell) {
        return {
          ...
          column: Warehouse.getColumnAt(warehouse, cell.column).map(function (
            col,
          ) {
    ...
    if (cells.length >= 1) {
      ...
      getSelectedCells(cells).each(function (selectedCells) {
        each(selectedCells, function (item) {
          ...
          var modifier = isSingleCell
            ? DomModifier.normal(editor, cellElm)
            : DomModifier.ifTruthy(editor, cellElm);
          var colModifier = item.column
            .map(function (col) {
              return isSingleCell
                ? DomModifier.normal(editor, col)
                : DomModifier.ifTruthy(editor, col);
            })
            .getOr(modifier);
  ...
  var getData = function (editor, cells) {
    var cellsData = getSelectedCells(cells).map(function (selectedCells) {
      return map(selectedCells, function (item) {
        return extractDataFromCellElement(
          editor,
          item.element,
  ...
  var applyRowData = function (editor, rows, oldData, data) {
    ...
    each(rows, function (rowElm) {
      ...
      var modifier = isSingleRow
        ? DomModifier.normal(editor, rowElm)
        : DomModifier.ifTruthy(editor, rowElm);
  ...
  var open$1 = function (editor) {
    ...
    var rowsData = map(rows, function (rowElm) {
      return extractDataFromRowElement(
        editor,
        rowElm.dom,
        hasAdvancedRowTab(editor),
      );
    });
  ...
  var applyDataToElement = function (editor, tableElm, data) {
    ...
    attrs.style = dom.serializeStyle(
      __assign(__assign({}, getDefaultStyles(editor)), styles),
    );
    dom.setAttribs(
      tableElm,
      __assign(__assign({}, getDefaultAttributes(editor)), attrs),
    );
  ...
  var open$2 = function (editor, insertNewTable) {
    ...
    if (insertNewTable === false) {
      ...
      if (tableElm) {
        data = extractDataFromTableElement(
          editor,
          tableElm,
    ...
    var dialogBody = hasAdvancedTableTab(editor)
      ? advancedForm()
      : nonAdvancedForm();
    editor.windowManager.open({
  ...
  var registerCommands = function (
    ...
    var eraseTable = function () {
      return getSelectionStartCellOrCaption$1(editor).each(function (
        cellOrCaption,
      ) {
        table(cellOrCaption, isRoot)
          .filter(not(isRoot))
          .each(function (table) {
    ...
    each$1(
      {
        ...
        mceTablePasteColBefore: function (_grid) {
          return pasteOnSelection(
            actions.pasteColsBefore,
            clipboard.getColumns,
          );
        },
    ...
    editor.addCommand('mceTableColType', function (_ui, args) {
      return get$1(args, 'type').each(function (type) {
        return actOnSelection(
          type === 'th'
            ? actions.makeColumnsHeader
            : actions.unmakeColumnsHeader,
        );
      });
    });
    each$1(
  ...
  var next = function (current, isRoot) {
    ...
    return detection.fold(
      ...
      function (info) {
        return info.index + 1 < info.all.length
          ? CellLocation.middle(current, info.all[info.index + 1])
          : CellLocation.last(current);
      },
    );
  };
  ...
  var caretPositionFromPoint = function (doc, x, y) {
    return Optional.from(doc.dom.caretPositionFromPoint(x, y)).bind(function (
      pos,
    ) {
  ...
  var setLegacyRtlRange = function (
    win,
    selection,
    start,
    soffset,
    ...
    if (!eq$1(start, finish)) {
      return identify(start, finish, isRoot).bind(function (cellSel) {
        ...
        if (boxes.length > 0) {
          ...
          return Optional.some(
            Response.create(
              Optional.some(makeSitus(start, 0, start, getEnd(start))),
              true,
            ),
          );
        } else {
  ...
  var left = function () {
    ...
    var first = function (children) {
      return children.length > 0
        ? Optional.some(children[children.length - 1])
        : Optional.none();
    };
    return {
      sibling: sibling,
  ...
  var hone = function (universe, item, predicate, mode, direction, isRoot) {
    ...
    return next.bind(function (n) {
      if (isRoot(n.item)) {
        ...
        return predicate(n.item)
          ? Optional.some(n.item)
          : hone(universe, n.item, predicate, n.mode, direction, isRoot);
      }
    });
  };
  ...
  var verify = function (
    ...
    return closest$1(after, 'td,th', isRoot)
      .bind(function (afterCell) {
        return closest$1(before, 'td,th', isRoot).map(function (beforeCell) {
          if (!eq$1(afterCell, beforeCell)) {
            return sharedOne$1(isRow, [afterCell, beforeCell]).fold(
              function () {
            ...
            return eq$1(after, afterCell) && getEnd(afterCell) === afterOffset
              ? failure(beforeCell)
              : adt$6.none('in same cell');
          }
        });
      })
  ...
  var gatherer = function (cand, gather, isRoot) {
    return gather(cand, isRoot).bind(function (target) {
      return isText(target) && get$4(target).trim().length === 0
        ? gatherer(target, gather, isRoot)
  ...
  var tryBr = function (isRoot, element, offset, direction) {
    var target = isBr(element)
      ? handleBr(isRoot, element, direction)
      : handleParent(isRoot, element, offset, direction);
  ...
  var getEntireBox = function (bridge, element) {
    if (isElement(element)) {
      ...
      return bridge
        .getRangedRect(element, 0, element, getEnd(element))
        .map(toCaret);
    } else {
      return Optional.none();
    }
  ...
  var JUMP_SIZE = 5;
  var NUM_RETRIES = 100;
  var adt$7 = Adt.generate([{ none: [] }, { retry: ['caret'] }]);
  var isOutside = function (caret, box) {
    return (
      caret.left < box.left ||
      Math.abs(box.right - caret.left) < 1 ||
  ...
  var adjustDown = function (bridge, element, guessBox, original, caret) {
    ...
    if (Math.abs(guessBox.bottom - original.bottom) < 1) {
      ...
      return inOutsideBlock(bridge, element, caret)
        ? adt$7.retry(translate(lowerCaret, JUMP_SIZE, 0))
        : adt$7.none();
    }
  };
  var adjustUp = function (bridge, element, guessBox, original, caret) {
    ...
    if (Math.abs(guessBox.top - original.top) < 1) {
      ...
      return inOutsideBlock(bridge, element, caret)
        ? adt$7.retry(translate(higherCaret, JUMP_SIZE, 0))
        : adt$7.none();
    }
  };
  var upMovement = {
  ...
  var adjustTil = function (bridge, movement, original, caret, numRetries) {
    ...
    return bridge
      .situsFromPoint(caret.left, movement.point(caret))
      .bind(function (guess) {
        return guess.start.fold(
          Optional.none,
          function (element) {
            return getEntireBox(bridge, element)
              .bind(function (guessBox) {
                return movement
                  .adjuster(bridge, element, guessBox, original, caret)
                  .fold(Optional.none, function (newCaret) {
                    return adjustTil(
                      bridge,
                      movement,
                      original,
  ...
  var retry = function (movement, bridge, caret) {
    ...
    var adjusted = adjustTil(bridge, movement, caret, moved, NUM_RETRIES).getOr(
      moved,
    );
    return checkScroll(movement, adjusted, bridge).fold(
      ...
      function (delta) {
        ...
        return bridge.situsFromPoint(
          adjusted.left,
          movement.point(adjusted) - delta,
        );
      },
    );
  };
    ...
    return bridge.getSelection().bind(function (sel) {
      return tryBr(isRoot, sel.finish, sel.foffset, direction).fold(
        ...
        function (brNeighbour) {
          ...
          var analysis = BeforeAfter.verify(
            bridge,
            sel.finish,
            sel.foffset,
            range.finish,
    ...
    return closest$1(initial, 'td,th', isRoot).bind(function (start) {
      return closest$1(start, 'table', isRoot).bind(function (table) {
        ...
        return handle$2(bridge, isRoot, direction).bind(function (range) {
          return closest$1(range.finish, 'td,th', isRoot).map(function (
            finish,
          ) {
            return {
              start: start,
      ...
      return closest$1(startRow, 'table', isRoot).bind(function (table) {
        ...
        if (eq$1(startRow, rows[0])) {
          return seekLeft$1(
            ...
            return Response.create(
              Optional.some(makeSitus(last, lastOffset, last, lastOffset)),
              true,
            );
          });
        } else {
      ...
      return closest$1(startRow, 'table', isRoot).bind(function (table) {
        ...
        if (eq$1(startRow, rows[rows.length - 1])) {
          return seekRight$1(
            ...
            return Response.create(
              Optional.some(makeSitus(first, 0, first, 0)),
              true,
            );
          });
        } else {
      ...
      cursor.on(function (start) {
        ...
        findCell(event.target, isRoot).each(function (finish) {
          identify(start, finish, isRoot).each(function (cellSel) {
            ...
            if (
              boxes.length > 1 ||
              (boxes.length === 1 && !eq$1(start, finish))
            ) {
              annotations.selectRange(
                container,
                boxes,
  ...
  var keyboard = function (win, container, isRoot, annotations) {
    ...
    var keydown = function (event, start, soffset, finish, foffset, direction) {
      ...
      var handler = retrieve(container, annotations.selectedSelector).fold(
        function () {
              ...
              return navigation.fold(
                function () {
                  return getEdges(
                    ...
                    var relative =
                      isDown(keycode) || direction.isForward(keycode)
                        ? Situ.after
                        : Situ.before;
    ...
    var keyup = function (event, start, soffset, finish, foffset) {
      return retrieve(container, annotations.selectedSelector).fold(
        function () {
          var realEvent = event.raw;
          var keycode = realEvent.which;
          var shiftKey = realEvent.shiftKey === true;
  ...
  var getUpOrLeftCells = function (grid, selectedCells, generators) {
    ...
    return bind(upDetails, function (detail) {
      var slicedCells = detail.cells.slice(
        0,
        selectedCells[selectedCells.length - 1].column + 1,
      );
  ...
  var getDownOrRightCells = function (grid, selectedCells, generators) {
    var downGrid = grid.slice(
      selectedCells[0].row + selectedCells[0].rowspan - 1,
      grid.length,
    );
    var downDetails = toDetailList(downGrid, generators);
    return bind(downDetails, function (detail) {
      var slicedCells = detail.cells.slice(
        selectedCells[0].column + selectedCells[0].colspan - 1,
        detail.cells.length,
      );
      return map(slicedCells, function (cell) {
        return cell.element;
  ...
  var hasInternalTarget = function (e) {
    return (
      has$1(SugarElement.fromDom(e.target), 'ephox-snooker-resizer-bar') ===
      false
    );
  };
  function CellSelection(editor, lazyResize, selectionTargets) {
    ...
    var annotations = SelectionAnnotation.byAttr(
      ephemera,
      onSelection,
      onClear,
    );
  ...
  var getSelectionTargets = function (editor, selections) {
    ...
    var findTargets = function () {
      return getSelectionStartCellOrCaption(getSelectionStart(editor)).bind(
        function (cellOrCaption) {
    ...
    var onSetup = function (api, isDisabled) {
      ...
      return function () {
        changeHandlers.set(
          filter(changeHandlers.get(), function (h) {
            return h !== handler;
          }),
        );
      };
    ...
    var onSetupPasteable = function (getClipboardData) {
      return function (api) {
        return onSetup(api, function (targets) {
          return (
            name(targets.element) === 'caption' || getClipboardData().isNone()
          );
        });
      };
    };
  ...
  function Plugin(editor) {
    ...
    var cellSelection = CellSelection(
      editor,
