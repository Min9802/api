...
(function () {
  ...
  var getValue = function (data, metaData, sourceInput) {
    return function (prop) {
      ...
      var getFromValueFirst = function () {
        return getFromData().bind(function (child) {
          return isObject(child)
            ...
            : getFromMetaData().orThunk(function () {
      ...
      return (
        (_a = {}),
        (_a[prop] = (
          prop === sourceInput ? getFromValueFirst() : getFromMetaFirst()
        ).getOr('')),
        _a
      );
    };
  ...
  var unwrap = function (data, sourceInput) {
    ...
    return __assign(
      __assign(
        __assign(
          __assign(__assign({}, get('source')), get('altsource')),
          get('poster'),
        ),
        get('embed'),
      ),
  ...
  var submitForm = function (prevData, newData, editor) {
    ...
    if (
      newData.embed &&
      (prevData.source === newData.source || isCached(newData.source))
    ) {
