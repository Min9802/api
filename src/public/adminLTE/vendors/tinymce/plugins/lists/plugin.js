...
(function () {
  ...
  var cleanListProperties = function (entry) {
    entry.listAttributes = filter$1(
      entry.listAttributes,
      function (_value, key) {
        return key !== 'start';
      },
    );
  ...
  var closestSiblingEntry = function (entries, start) {
    ...
    return findUntil(reverse(entries.slice(0, start)), matches, until).orThunk(
      function () {
  ...
  var getSelectedTextBlocks = function (editor, rng, root) {
    ...
    global$5.each(siblings, function (node) {
      ...
      if (global$7.isBookmarkNode(node)) {
        if (
          isTextBlock(editor, nextSibling) ||
  ...
  var isValidLists = function (list1, list2) {
    return (
      list1 && list2 && isListNode(list1) && list1.nodeName === list2.nodeName
    );
  };
  ...
  var mergeLiElements = function (dom, fromElm, toElm) {
    ...
    var nestedLists = contains
      ? dom.getParents(fromElm, isListNode, toElm)
      : [];
    dom.remove(fromElm);
