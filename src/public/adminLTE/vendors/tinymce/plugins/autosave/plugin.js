...
(function () {
  ...
  var parse = function (timeString, defaultTime) {
    ...
    return (
      (parsedTime[2] ? multiples[parsedTime[2]] : 1) * parseInt(toParse, 10)
    );
  };
  ...
  var getAutoSavePrefix = function (editor) {
    ...
    return editor
      .getParam('autosave_prefix', 'tinymce-autosave-{path}{query}{hash}-{id}-')
  ...
  var hasDraft = function (editor) {
    var time =
      parseInt(global$2.getItem(getAutoSavePrefix(editor) + 'time'), 10) || 0;
  ...
  var makeSetupHandler = function (editor) {
    return function (api) {
      ...
      return function () {
        return editor.off(
          'StoreDraft RestoreDraft RemoveDraft',
          editorEventCallback,
        );
      };
    };
