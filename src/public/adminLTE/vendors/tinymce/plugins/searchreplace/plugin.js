...
(function () {
  ...
  var isSimpleBoundary = function (dom, node) {
    return (
      dom.isBlock(node) ||
      has(dom.schema.getShortEndedElements(), node.nodeName)
    );
  };
  ...
  var isHidden = function (dom, node) {
    return (
      !dom.isBlock(node) &&
      has(dom.schema.getWhiteSpaceElements(), node.nodeName)
    );
  };
  var isBoundary = function (dom, node) {
    return (
      isSimpleBoundary(dom, node) ||
      isContentEditableFalse(dom, node) ||
      isHidden(dom, node) ||
      isContentEditableTrueInCef(dom, node)
    );
  };
  ...
  var walk = function (
    ...
    while (next) {
      ...
      if (isCefNode || isHidden(dom, next)) {
        var stopWalking = isCefNode
          ? callbacks.cef(next)
          : callbacks.boundary(next);
        if (stopWalking) {
          break;
        } else {
          next = walkerFn(true);
  ...
  var collectRangeSections = function (dom, rng) {
    ...
    return collect(
      ...
      endNode,
      {
        ...
        cef: function (node) {
          var sections = bind(
            descendants(SugarElement.fromDom(node), '*[contenteditable=true]'),
            function (e) {
  ...
  var find$1 = function (pattern, sections) {
    return bind(sections, function (section) {
      ...
      var positions = find(
        ...
        pattern,
        section.sOffset,
        content.length - section.fOffset,
      );
  ...
  var findAndMarkInSelection = function (
    dom,
    pattern,
    selection,
    replacementNode,
  ...
  var moveSelection = function (editor, currentSearchState, forward) {
    ...
    dom.removeClass(
      findSpansByIndex(editor, searchState.index),
  ...
  var next = function (editor, currentSearchState) {
    ...
    currentSearchState.set(
      __assign(__assign({}, currentSearchState.get()), { index: index }),
    );
  };
  ...
  var open = function (editor, currentSearchState) {
    ...
    var selectedText = global$1.trim(
      editor.selection.getContent({ format: 'text' }),
    );
    function updateButtonStates(api) {
      var updateNext = hasNext(editor, currentSearchState)
        ? api.enable
        : api.disable;
      updateNext('next');
      var updatePrev = hasPrev(editor, currentSearchState)
        ? api.enable
        : api.disable;
      updatePrev('prev');
    }
    ...
    var focusButtonIfRequired = function (api, name) {
      if (
        global$3.browser.isSafari() &&
        global$3.deviceType.isTouch() &&
