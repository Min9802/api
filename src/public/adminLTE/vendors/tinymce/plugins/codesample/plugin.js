...
(function () {
      ...
      })()(
        {
          ...
          5: [
            function (require, module, exports) {
              (function (Prism) {
                ...
                var identifier = replace(
                  /(?!<<0>>)<<1>>(?:\s*\.\s*<<1>>)*/.source,
                  [nonTypeKeywords, genericName],
                );
                ...
                var mInterpolation = replace(
                  /\{(?!\{)(?:(?![}:])<<0>>)*<<1>>?\}/.source,
                  [mInterpolationRound, formatString],
                );
                ...
                var sInterpolation = replace(
                  /\{(?!\{)(?:(?![}:])<<0>>)*<<1>>?\}/.source,
                  [sInterpolationRound, formatString],
                );
                    ...
                    interpolation: {
                      ...
                      inside: {
                        'format-string': {
                          pattern: re(
                            /(^\{(?:(?![}:])<<0>>)*)<<1>>(?=\}$)/.source,
                            [interpolationRound, formatString],
                          ),
                          lookbehind: true,
                          inside: { punctuation: /^:/ },
                        },
              ...
              Prism.languages.dotnet = Prism.languages.cs =
            ...
            function (require, module, exports) {
              (function (Prism) {
                ...
                Prism.languages.java = Prism.languages.extend('clike', {
                  'class-name': [className, /\b[A-Z]\w*(?=\s+\w+\s*[;,=())])/],
                  keyword: keywords,
                  function: [
          ...
          10: [
            function (require, module, exports) {
              ...
              Prism.languages.markup['tag'].inside['attr-value'].inside[
                'entity'
  ...
  var open = function (editor) {
    ...
    var defaultLanguage = head(languages).fold(
      function () {
        return '';
      },
      function (l) {
  ...
  var setup = function (editor) {
    ...
    editor.on('SetContent', function () {
      var unprocessedCodeSamples = $('pre')
        .filter(trimArg(isCodeSample))
