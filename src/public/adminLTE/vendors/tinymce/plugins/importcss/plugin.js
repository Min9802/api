...
(function () {
  ...
  var isSkinContentCss = function (editor, href) {
    ...
    if (skin) {
      ...
      var skinUrl = skinUrlBase
        ? editor.documentBaseURI.toAbsolute(skinUrlBase)
        : global$2.baseURL + '/skins/ui/' + skin;
      ...
      return (
        href ===
          skinUrl +
            ...
            '.min.css' || href.indexOf(contentSkinUrlPart) !== -1
      );
    }
    return false;
  };
    ...
    editor.on('init', function (_e) {
      ...
      global$4.each(
        getSelectors(
          editor,
          editor.getDoc(),
          compileFilter(getFileFilter(editor)),
        ),
        function (selector) {
