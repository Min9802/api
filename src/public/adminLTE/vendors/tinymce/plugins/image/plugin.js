...
(function () {
  ...
  var create = function (normalizeCss, data) {
    ...
    write(
      normalizeCss,
      __assign(__assign({}, data), { caption: false }),
      image,
    );
  ...
  var insertOrUpdateImage = function (editor, partialData) {
    ...
    if (image) {
      ...
      insertImageAtCaret(
        editor,
        __assign(__assign({}, defaultData()), partialData),
      );
    }
  };
  ...
  function Uploader(settings) {
    ...
    var upload = function (blobInfo) {
      return !settings.url && isDefaultHandler(settings.handler)
        ? global$2.reject('Upload url missing from the settings.')
  ...
  var collect = function (editor) {
    ...
    var prependURL = Optional.some(getPrependUrl(editor)).filter(function (
      preUrl,
    ) {
  ...
  var calcVSpace = function (css) {
    ...
    return matchingTopBottom
      ? removePixelSuffix(String(css['margin-top']))
      : '';
  };
  var calcHSpace = function (css) {
    ...
    return matchingLeftRight
      ? removePixelSuffix(String(css['margin-right']))
      : '';
  };
  var calcBorderWidth = function (css) {
    return css['border-width']
      ? removePixelSuffix(String(css['border-width']))
      : '';
  };
    ...
    editor.ui.registry.addContextMenu('image', {
      update: function (element) {
        return isFigure(element) ||
          (isImage(element) && !isPlaceholderImage(element))
          ? ['image']
          : [];
      },
    });
