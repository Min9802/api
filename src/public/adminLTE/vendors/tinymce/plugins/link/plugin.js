...
(function () {
  ...
  var applyLinkOverrides = function (editor, linkAttrs) {
    ...
    if (
      !(getRelList(editor).length > 0) &&
      allowUnsafeLinkTarget(editor) === false
    ) {
    ...
    if (
      Optional.from(newLinkAttrs.target).isNone() &&
      getTargetList(editor) === false
    ) {
  ...
  var init = function (initialData, linkCatalog) {
    ...
    var getTitleFromUrlChange = function (url) {
      return someIf(
        persistentData.title.length <= 0,
        Optional.from(url.meta.title).getOr(''),
      );
    };
    var getTextFromUrlChange = function (url) {
      return someIf(
        persistentData.text.length <= 0,
        Optional.from(url.meta.text).getOr(url.value),
      );
    };
  ...
  var tryProtocolTransform = function (
    ...
    return function (data) {
      var url = data.href;
      var suggestProtocol =
        (assumeExternalTargets === 1 && !hasProtocol(url)) ||
  ...
  var extractFromAnchor = function (editor, anchor) {
    ...
    var text = onlyText
      ...
      : Optional.none();
    var url = anchor
      ? Optional.some(dom.getAttrib(anchor, 'href'))
      : Optional.none();
    var target = anchor
      ? Optional.from(dom.getAttrib(anchor, 'target'))
      : Optional.none();
  ...
  var makeDialog = function (settings, onSubmit, editor) {
    ...
    var displayText = settings.anchor.text
      .map(function () {
        return {
          name: 'text',
          type: 'input',
    ...
    return {
      ...
      onChange: function (api, _a) {
        var name = _a.name;
        dialogDelta
          .onChange(api.getData, { name: name })
