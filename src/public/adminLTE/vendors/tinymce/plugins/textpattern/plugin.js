...
(function () {
  ...
  var denormalizePattern = function (pattern) {
    if (pattern.type === 'block-command') {
      ...
      return {
        ...
        format:
          pattern.format.length === 1 ? pattern.format[0] : pattern.format,
      };
    }
  };
  ...
  var get$1 = function (patternsState) {
    ...
    var getPatterns = function () {
      return __spreadArrays(
        map(patternsState.get().inlinePatterns, denormalizePattern),
        map(patternsState.get().blockPatterns, denormalizePattern),
      );
    };
    return {
  ...
  var isBlockFormatName = function (name, formatter) {
    ...
    return (
      isArray(formatSet) &&
      head(formatSet).exists(function (format) {
  ...
  var getParentBlock = function (editor, rng) {
    var parentBlockOpt = Optional.from(
      editor.dom.getParent(rng.startContainer, editor.dom.isBlock),
    );
  ...
  var generatePathRange = function (
    root,
    startNode,
    startOffset,
    endNode,
  ...
  var resolvePath = function (root, path) {
    ...
    var resolvedNode = foldl(
      nodePath,
      function (optNode, index) {
  ...
  var applyPattern = function (editor, match) {
    ...
    var rng = resolvePathRange(dom.getRoot(), match.range).getOrDie(
  ...
  var createMarker = function (dom, markerPrefix, pathRange) {
    var rng = resolvePathRange(dom.getRoot(), pathRange).getOrDie(
      'Unable to resolve path range',
    );
  ...
  var matchesPattern = function (dom, block, patternContent) {
    return function (element, offset) {
      ...
      var startEndIndex = searchText.lastIndexOf(
    ...
    return startSpot.bind(function (spot) {
      if (spot.offset >= startPattern.length) {
        ...
        return scanLeft(spot.container, offset, block)
          ...
          .orThunk(function () {
            return findPatternStartFromSpot(
              dom,
              pattern,
              block,
  ...
  var findPattern$1 = function (editor, block, details) {
    ...
    return scanLeft(
      ...
      var endPathRng = generatePathRange(
        root,
        spot.container,
        spot.offset,
        endNode,
  ...
  var applyPatternWithContent = function (
    editor,
    pattern,
    startMarker,
    endMarker,
  ...
  var addMarkers = function (dom, matches) {
    ...
    var matchesWithEnds = foldr(
      matches,
      function (acc, match) {
        var endMarker = createMarker(
          dom,
          markerPrefix + ('_end' + acc.length),
          match.endRng,
        );
        return acc.concat([
          __assign(__assign({}, match), { endMarker: endMarker }),
        ]);
      },
      [],
    );
    return foldr(
      ...
      function (acc, match) {
        ...
        var startMarker = isReplacementPattern(match.pattern)
          ? match.endMarker
          : createMarker(dom, markerPrefix + ('_start' + idx), match.startRng);
        return acc.concat([
          __assign(__assign({}, match), { startMarker: startMarker }),
        ]);
      },
      [],
    );
  ...
  var handleEnter = function (editor, patternSet) {
    ...
    var inlineMatches = findPatterns$1(
      editor,
  ...
  var checkKeyCode = function (codes, event) {
    return checkKeyEvent(codes, event, function (code, event) {
      return (
        code === event.keyCode && global$2.modifierPressed(event) === false
      );
    });
  };
