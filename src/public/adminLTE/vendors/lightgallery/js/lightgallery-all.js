  ...
  (function () {
    ...
    Video.prototype.init = function () {
      ...
      _this.core.$el.on(
        ...
        onAferAppendSlide.bind(this),
      );

      if (
        _this.core.doCss() &&
        _this.core.$items.length > 1 &&
        (_this.core.s.enableSwipe || _this.core.s.enableDrag)
      ) {
    ...
    Video.prototype.loadVideo = function (
      ...
      if (_this.core.s.dynamic) {
        ...
        videoTitle =
          _this.core.$items.eq(_this.core.index).attr('title') ||
          _this.core.$items
            .eq(_this.core.index)
            .find('img')
            .first()
  ...
  (function () {
    ...
    Zoom.prototype.init = function () {
      ...
      var zoom = function (scaleVal) {
        ...
        var offsetY =
          ($(window).height() - $image.prop('offsetHeight')) / 2 +
          $(window).scrollTop();
      ...
      var actualSize = function (event, $image, index, fromIcon) {
        ...
        if (_this.core.s.dynamic) {
          nw =
            _this.core.s.dynamicEl[index].width || $image[0].naturalWidth || w;
        } else {
          nw =
    ...
    Zoom.prototype.getPossibleDragCords = function ($image, rotateValue) {
      var minY =
        (this.core.$outer.find('.lg').height() -
          this.getImageSize($image, rotateValue, 'y')) /
        2;
      var maxY = Math.abs(
        this.getImageSize($image, rotateValue, 'y') *
          Math.abs($image.attr('data-scale')) -
          this.core.$outer.find('.lg').height() +
          minY,
      );
      var minX =
        (this.core.$outer.find('.lg').width() -
          this.getImageSize($image, rotateValue, 'x')) /
        2;
      var maxX = Math.abs(
        this.getImageSize($image, rotateValue, 'x') *
          Math.abs($image.attr('data-scale')) -
    ...
    Zoom.prototype.zoomSwipe = function () {
      ...
      _this.core.$slide.on('touchmove.lg', function (e) {
        if (_this.core.$outer.hasClass('lg-zoomed')) {
          ...
          if (allowY) {
            distanceY =
              -Math.abs(_$el.attr('data-y')) +
              (endCoords.y - startCoords.y) *
                _this.getModifier(rotateValue, 'Y', rotateEl);
          } else {
          ...
          if (allowX) {
            distanceX =
              -Math.abs(_$el.attr('data-x')) +
              (endCoords.x - startCoords.x) *
                _this.getModifier(rotateValue, 'X', rotateEl);
          } else {
    ...
    Zoom.prototype.touchendZoom = function (
      ...
      var distanceY =
        ...
        (endCoords.y - startCoords.y) *
          _this.getModifier(rotateValue, 'Y', rotateEl);
      var possibleDragCords = _this.getPossibleDragCords(
        $image,
        Math.abs(rotateValue),
      );
      if (
...
(function (root, factory) {
  ...
  (function () {
    ...
    Share.prototype.init = function () {
      ...
      var shareHtml =
        '<button type="button" aria-label="Share" id="lg-share" class="lg-icon" aria-haspopup="true" aria-expanded="false">' +
